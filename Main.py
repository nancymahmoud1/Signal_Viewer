...
class AddSignalDialog(QtWidgets.QDialog):
    ...
    def retranslateUi(self):
        self.setWindowTitle("Choose Graph to ADD")
        self.load_file_button.setText("Graph 1")
        self.color_ok_button.setText("Graph 2")
        self.color_cancel_button.setText("Cancel")
        self.label.setText("Choose Graph to ADD")
...
class ReportDialog(QDialog):
    ...
    def get_stats(self):
        ...
        if hasattr(self.main_window, 'signals_data_ax1'):
            signal_data_1 = self.main_window.signals_data_ax1[0]
        ...
        if self.main_window.is_second_graph_active:
            ...
            if hasattr(self.main_window, 'signals_data_ax2'):
                signal_data_2 = self.main_window.signals_data_ax2
...
class Ui_MainWindow(object):
    ...
    def initialize_rectangular_graph(self, content_widget, play_pause_button_1, play_pause_button_2, reset_button,
                                     link_button, unified_play_pause_button, add_signal_button, add_graph_button,
        ...
        add_signal_button.setFixedSize(150, 40)
        add_signal_button.setFont(font)  # Set Times New Roman font
        add_signal_button.setCursor(QtGui.QCursor(QtCore.Qt.PointingHandCursor))
        add_signal_button.setStyleSheet(f"""
                    QPushButton {{
                        color: white;
                        background-color: rgb(40,40,40);
                    }}
        ...
        button_layout.addWidget(add_signal_button, alignment=QtCore.Qt.AlignHCenter)
        self.add_signal_button = add_signal_button
    ...
    def setup_rectangular_page(self):
        if not hasattr(self, 'rectangular_initialized') or not self.rectangular_initialized:
            ...
            # Set up the timer for playing and updating all signals on ax1
            self.timer_1 = QtCore.QTimer()
            self.timer_1.timeout.connect(lambda: self.update_signals(self.ax1, self.signals_data_ax1, self.line_plots_ax1, self.timer_1, 'ax1'))

            self.timer_2 = QtCore.QTimer()
            self.timer_2.timeout.connect(lambda :self.update_signals(self.ax2, self.signals_data_ax2, self.line_plots_ax2, self.timer_2, 'ax2'))
            ...
            self.signals_data_ax1.append(np.loadtxt(filename))
            self.index_1 = 0
            self.window_size_1 = 400

            self.window_size_2 = 400

            # Plot the first graph initially (Rectangular)
            self.ax1 = self.figure_1.add_subplot(111)
            self.ax1.set_xlim(0, self.window_size_1)
            self.ax1.set_ylim(min(np.min(sig) for sig in self.signals_data_ax1),
                              max(np.max(sig) for sig in self.signals_data_ax1))

            # Draw initial empty lines
            for signal_data in self.signals_data_ax1:
                line_plot, = self.ax1.plot([], [], color=self.plot_color)
                self.line_plots_ax1.append(line_plot)
    ...
    def add_signal(self):
        """Open file dialog to select and add a new signal to ax1 or ax2."""
        # Show the dialog for selecting the axis
        dialog = AddSignalDialog(self.centralwidget)
        result = dialog.exec_()

        if result == QDialog.Accepted:

            # Open a file dialog for the user to select a new signal file
            options = QtWidgets.QFileDialog.Options()
            filename, _ = QtWidgets.QFileDialog.getOpenFileName(
                None, "Select Signal File", "", "Text Files (*.txt);;All Files (*)", options=options)

            # If a file is selected, load and add it
            if filename:
                # Load the signal data
                signal_data = np.loadtxt(filename)

                # Choose a color for the new signal
                selected_color = self.choose_color()
                if not selected_color:
                    print("No color selected. Signal not added.")
                    return

                if dialog.selected_signal == 1:
                    # Add to ax1
                    self.signals_data_ax1.append(signal_data)

                    # Create a new line plot for the new signal in ax1
                    line_plot, = self.ax1.plot([], [], color=selected_color)
                    self.line_plots_ax1.append(line_plot)

                    # Update limits for ax1
                    self.ax1.set_ylim(min(np.min(sig) for sig in self.signals_data_ax1),
                                      max(np.max(sig) for sig in self.signals_data_ax1))
                    self.ax1.set_xlim(0, self.window_size_1)

                    # Update the canvas for ax1
                    self.canvas_1.draw()

                elif dialog.selected_signal == 2 and self.is_second_graph_active:
                    # Add to ax2
                    self.signals_data_ax2.append(signal_data)

                    # Create a new line plot for the new signal in ax2
                    line_plot_2, = self.ax2.plot([], [], color=selected_color)
                    self.line_plots_ax2.append(line_plot_2)

                    # Update limits for ax2
                    self.ax2.set_ylim(min(np.min(sig) for sig in self.signals_data_ax2),
                                      max(np.max(sig) for sig in self.signals_data_ax2))
                    self.ax2.set_xlim(0, self.window_size_2)

                    # Update the canvas for ax2
                    self.canvas_2.draw()

                print(f"Signal added to {dialog.selected_signal}.")

    def choose_color(self):
        """Open a color picker dialog and return the selected color."""
        color = QColorDialog.getColor()
        if color.isValid():
            return color.name()  # Returns the color in hex format
        return None  # Return None if no valid color is chosen

    def update_signals(self, ax, signals_data, line_plots, timer, axis_name):
        """Update all signals on a specified axis (ax1 or ax2) at each timer tick."""

        for i, (signal_data, line_plot) in enumerate(zip(signals_data, line_plots)):
            # Create unique names for the index and window size attributes based on axis and signal number
            index_name = f'index_{axis_name}_{i + 1}'
            window_size_attr = f'window_size_{axis_name}_{i + 1}'

            # Get the current index and window size for each signal
            window_size = getattr(self, window_size_attr, 400)
            current_index = getattr(self, index_name, 0)

            # Update the signal plot
            new_index = self._update_individual_signal(signal_data, current_index, window_size, line_plot, ax, timer)

            # Update the stored index for this signal
            setattr(self, index_name, new_index)

    def _update_individual_signal(self, signal_data, index, window_size, line_plot, ax, timer):
        """Update a single signal data plot without clearing previous points."""

        if signal_data is None or line_plot is None:
            timer.stop()
            print(f"Update skipped for {line_plot} because it is not initialized.")
            return index  # Maintain current index if data is not initialized

        if index >= len(signal_data):
            timer.stop()
            # Set play button to "Play" if playback has ended
            self.play_pause_button_1.setText("Play ▶")
            return 0  # Reset index to 0 at the end of the signal

        # Get the current plot data for x and y
        current_x_data = line_plot.get_xdata()
        current_y_data = line_plot.get_ydata()

        # Set step size for updates
        step_size = getattr(self, 'step_size', 2)

        # Append new data for the plot
        new_x_data = np.append(current_x_data, np.arange(index, index + step_size))
        new_y_data = np.append(current_y_data, signal_data[index:index + step_size])

        # Implement a moving window effect by keeping only the latest 'window_size' points
        if len(new_x_data) > window_size:
            ax.set_xlim(new_x_data[-window_size], new_x_data[-1])
        ...
        return index + step_size  # Move forward by step_size
    ...
    def toggle_add_rectangular_graph(self, button):
        ...
        if not self.is_second_graph_active:  # Add the second graph (Rectangular Signal 2)
            ...
            self.signals_data_ax2.append(np.loadtxt(filename2))
            self.index_2 = 0
            self.window_size_2 = 400
            ...
            self.selector_ax1 = RectangleSelector(
                self.ax1,
                onselect=self.on_select_range_ax1,
                useblit=True,
            ...
            # Optionally, add a similar selector for ax2 if needed
            self.selector_ax2 = RectangleSelector(
                self.ax2,
                onselect=self.on_select_range_ax2,
                useblit=True,
                button=[1],
                minspanx=5, minspany=5,
                spancoords='pixels',
            ...
            # self.ax2.set_ylim(min(np.min(sig) for sig in self.signals_data_ax2),
            #                   max(np.max(sig) for sig in self.signals_data_ax2))
            for signal_data2 in self.signals_data_ax2:
                line_plot_2, = self.ax2.plot([], [], color=self.plot_color)
                self.line_plots_ax2.append(line_plot_2)
    ...
    def on_select_range_ax1(self, eclick, erelease):
        """Called when a range is selected on ax1."""
        self.selected_range_ax1 = (int(eclick.xdata), int(erelease.xdata))
        print(f"Selected range for ax1: {self.selected_range_ax1}")

    def on_select_range_ax2(self, eclick, erelease):
        """Called when a range is selected on ax2."""
        self.selected_range_ax2 = (int(eclick.xdata), int(erelease.xdata))
        print(f"Selected range for ax2: {self.selected_range_ax2}")
    ...
    def reset_rectangular_signals(self):
        ...
        # Reset the line plots for ax1
        for line_plot in self.line_plots_ax1:
            line_plot.set_data([], [])  # Clear Rectangular signal plot

        # Reset the x-axis and y-limits for ax1
        if hasattr(self, 'ax1'):
            self.ax1.set_xlim(0, self.window_size_1)
            if self.signals_data_ax1:  # Ensure there's data before accessing
                self.ax1.set_ylim(min(np.min(sig) for sig in self.signals_data_ax1),
                                  max(np.max(sig) for sig in self.signals_data_ax1))
            self.ax1.relim()
            self.ax1.autoscale_view()

        # Reset the line plots for ax2 if it is active
        if self.is_second_graph_active and hasattr(self, 'line_plots_ax2'):
            for line_plot_2 in self.line_plots_ax2:
                line_plot_2.set_data([], [])  # Clear Rectangular signal 2 plot

            # Reset the x-axis and y-limits for ax2
            if hasattr(self, 'ax2'):
                self.ax2.set_xlim(0, self.window_size_2)
                if self.signals_data_ax2:  # Ensure there's data before accessing
                    self.ax2.set_ylim(min(np.min(sig) for sig in self.signals_data_ax2),
                                      max(np.max(sig) for sig in self.signals_data_ax2))
                self.ax2.relim()
        ...
        # 4. Redraw the axes for both plots
        if hasattr(self, 'ax1'):
            self.ax1.relim()
            self.ax1.autoscale_view()
        if hasattr(self, 'ax2'):
        ...
        # Redraw the canvases after resetting
        self.canvas_1.draw_idle()
    ...
    def merge_signals(self, signal_1_label, signal_2_label):
        ...
        # Plot both signals
        for signal_data in self.signals_data_ax1:
            line_plot, = self.ax1.plot(np.arange(len(signal_data)), signal_data)
            self.line_plots_ax1.append(line_plot)

        for signal_data_2 in self.signals_data_ax2:
            line_plot, = self.ax1.plot(np.arange(len(signal_data_2)), signal_data_2)
            self.line_plots_ax1.append(line_plot)
    ...
    def unmerge_signals(self, signal_1_label, signal_2_label):
        ...
        if self.signals_data_ax2 is not None:
            for signal_data_2 in self.signals_data_ax2:
                line_plot_2, = self.ax2.plot(np.arange(len(signal_data_2)), signal_data_2)
                self.line_plots_ax2.append(line_plot_2)
    ...
    def glue_signals(self):
        ...
        start_1, end_1 = self.selected_range_ax1  # Indices for signal on ax1
        start_2, end_2 = self.selected_range_ax2  # Indices for signal on ax2

        # Extract the segments from both signals
        part_1 = self.signals_data_ax1[0][start_1:end_1]
        part_2 = self.signals_data_ax2[0][start_2:end_2]
    ...
    def setup_RTS_page(self):
        if not hasattr(self, 'rts_initialized') or not self.rts_initialized:
            ...
            self.timer_1.timeout.connect(
                lambda: self.update_RTS_signal('signal_data_1','Time_data_1', 'index_1', self.window_size_1, self.line_plot_1,
                                               self.ax1,
                                               self.timer_1))
            ...
            self.line_plot_1, = self.ax1.plot(self.Time_data_1[:self.window_size_1],self.signal_data_1[:self.window_size_1], color=self.plot_color)
            self.ax1.tick_params(colors=self.label_color)
    ...
    def setup_mouse_events(self):
        ...
        if hasattr(self, 'canvas_1') and hasattr(self, 'canvas_2'):
            ...
            self.canvas_1.mpl_connect('button_press_event',
                                      lambda event: self.on_click(event, self.ax1, self.canvas_1, self.signals_data_ax1))

            # Setup mouse events for ax2 (canvas_2)
            self.canvas_2.mpl_connect('button_press_event',
                                      lambda event: self.on_click(event, self.ax2, self.canvas_2, self.signals_data_ax2))
    ...
    '''def setup_mouse_event(self):
        ...
        if hasattr(self, 'canvas_1') and hasattr(self, 'canvas_2'):
            ...
            self.canvas_1.mpl_connect('button_press_event',
                                      lambda event: self.on_click02(event, self.ax1, self.canvas_1, self.signals_data_ax1))
            ...
            self.canvas_2.mpl_connect('button_press_event',
                                      lambda event: self.on_click02(event, self.ax2, self.canvas_2, self.signals_data_ax2))
